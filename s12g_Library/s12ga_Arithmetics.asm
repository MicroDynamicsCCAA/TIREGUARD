	...
	title	"s12ga_Arithmetics  Copyright (C) 2004-2014, micro dynamics GmbH"
;------------------------------------------------------------------------------
;Module:	s12ga_Arithmetics.asm
;
...
;		Herkunft: s12xep_Arithmetics.asm
;		MUL3216S: Anpassung an HCS12- statt HCS12X-Befehlssatz
;
;12.01.2011	Function MUL3216S neu
;
;01.11.2010	Anpassung an MC9XEP100
;		Herkunft: s12p_Arithmetics.asm
;
;28.04.2010	Anpassung an MC9S12P128
;		Herkunft: s12c_Arithmetics.asm
;
...
MUL3216U:
	...
	RTS				;
					;
;------------------------------------------------------------------------------
;MUL3216S multipliziert eine vorzeichenbehaftete 32-bit Zahl mit einer
;vorzeichenbehafteten 16-bit Zahl und liefert ein vorzeichenbehaftetes
;64-bit Ergebnis.
;
;Eingangsparameter:	R[0..3]		Multiplikand MSB..LSB
;			R[6..7]		Multiplikator MSB..LSB
...
;------------------------------------------------------------------------------
					;
MUL3216S:
	LDAA	R8			;Register R8 retten
	PSHA				;
	MOVB	#1,R8			;Vorzeichenmerker auf Startwert
	TST	R0			;
	BPL	MUL3216S1		;wenn Multiplikand negativ,
	COM	R3			;dann
	COM	R2			;  Zweierkomplement des Multiplikanden bilden
	COM	R1			;
	COM	R0			;
	LDD	R2			;
	ADDD	#1			;
	...
	ADCA	#0			;
	STD	R0			;
	NEG	R8			;  Vorzeichenmerker aktualisieren
MUL3216S1:
	TST	R6			;wenn Multiplikator negativ,
	BPL	MUL3216S2		;
	COM	R7			;dann
	COM	R6			;  Zweierkomplement des Multiplikators bilden
	LDD	R6			;
	ADDD	#1			;
	STD	R6			;
	NEG	R8			;  Vorzeichenmerker aktualisieren
MUL3216S2:
	JSR	MUL3216U		;vorzeichenlose Multiplikation ausführen
	TST	R8			;wenn Ergebnis negativ,
	BPL	MUL3216S3		;
	COM	R7			;dann
	COM	R6			;  Zweierkomplement des Ergebnisses bilden
	COM	R5			;
	COM	R4			;
	COM	R3			;
	COM	R2			;
...
MUL3216S3:
	PULA				;
	STAA	R8			;Register R8 restaurieren
	RTS				;
					;
;------------------------------------------------------------------------------
