	...
	title 	"tg3Declarations  Copyright (C) 2005-2013, micro dynamics GmbH"
;------------------------------------------------------------------------------
...
;begin 02.07.2013
TIRE_RFSH_TIME: equ	1000		;Zeittakt für Identifier CAN-Ausgabe
;end
;
					;
;
...
;
;begin 02.07.2013
TIRE_RFSH_CT:	equ	TIRE_RFSH_TIME / LOOPTIME
;end
;
...
;-----------------------------------------------------------------------------
					;
 if BUS_CLK < 16000
C_TSCR2:	equ	00000011b	;No Interrupts, Free Running Counter, Prescaler = 8
CLK_PRESCALER:	equ	8		;Vorteiler = 8
 else
...
C_SYNR:		equ	0		;BUS_CLK = 8,0 MHz @ OSC_CLK = 16,0 MHz
C_REFDV:	equ	1		;
 endif
					;
;-----------------------------------------------------------------------------
...
C_FTS_FCLKDIV:	equ	(OSC_CLK * (5 + 1 / BUS_CLK) / 1000) - 1
 endif
...
SW_IDENT:	dc.b	"TIREGUARD IIIa  " ;jede Zeile umfasst 16 Zeichen
		...
		dc.b	"GmbH, CCAA      "
SW_VERSION:	dc.b	"S2.804.26/A,    " ;das Komma muss(!) angefügt werden
		dc.b	"02.07.2013      "
...
;		:			1  Alarmblinken
;bit 1..3	: reserviert
;bit 4		: Identifier CAN-Output	0  Identifier an CAN ausgeben
;		:			1  keine Identifier CAN-Ausgabe
;bit 5..7	: reserviert
;-----------------------------------------------------------------------------
					;
FUN_MODE:
		dc.b	00000001b	;Alarmblinken, Identifier CAN-Ausgabe
;end
;
					;
;-----------------------------------------------------------------------------
...
;
;begin 02.07.2013
		dc.w	0DE48H		;Descriptor 3	6F2h, 8 Bytes, senden
		dc.w	0DE68h		;Descriptor 4	6F3h, 8 Bytes, senden
;end
;
...
;begin 02.07.2013
TIRE_RFSH_CTR:
		ds.b	1		;DATA8: Zähler für Identifier-Ausgabezykluszeit
;end
;
					;
 ifne fDebug
