	...
	title	"s12c_MSCANv2  Copyright (C) 2005-2018, micro dynamics GmbH"
;------------------------------------------------------------------------------
;Module:	s12c_MSCANv2.asm
;
;Copyright:	(C) 2005-2018, micro dynamics GmbH
...
;Funktion CAN_CONFIG:
;Die Reihenfolge der Werte ist fest vorgegeben und kann nicht verändert werden.
;
...
;
;09.01.2018     Handhabung der Empfangs-Akzeptanzfilter überarbeitet:
;		CANx_TX_INT geändert
;		CANx_RX_INT geändert
;               CHK_FILTER neu
;		ADD_FILTER neu
;		CAN_CONFIG geändert
;
;		Fehler in CAN_REQUEST korrigiert:
;		Sprung nach CAN_REQUEST9 statt nach CAN_WRITE9
;
;24.11.2006
...
;Initialisierung:	Module 's12c_MSCANv2'
;
...
;
;begin 09.01.2018
	LDAA	#0FFh			;wenn Identifier = [7F8h..7FFh]
	CMPA	CAN0RXIDR0		;dann
	BNE	CAN0_RX_INT0		;
	JMP	CAN0_RX_INT9		;  Botschaft grundsätzlich ignorieren
					;
CAN0_RX_INT0:
	LDAA	R0			;Register R0 retten
	PSHA				;
...
;begin 09.01.2018
;------------------------------------------------------------------------------
;CHK_FILTER trägt einen noch nicht vorhandenen Akzeptanzwert ggf. in den
;augenblicklich adressierten Akzeptanzfilter ein.
;
;Eingangsparameter:	X		Zeiger auf Descriptor
;			Y               CAN Register-Basisadresse
;			B		CAN Register-Offset = Zeiger auf Akzeptanzfilter
;Ausgangsparameter:	A		0	= Wert eingetragen, fertig
;					<> 0	= kein Wert eingetragen
;veränderte Register:   CCR
;------------------------------------------------------------------------------
					;
CHK_FILTER:
	LDAA	B,Y			;wenn Akzeptanzwert schon vorhanden,
	CMPA	0,X			;dann
	BEQ	CHK_FILTER7		;  bereits fertig
	CMPA	#0FFh			;sonst wenn Akzeptanzfilter noch frei,
	BNE	CHK_FILTER8		;dann
	LDAA	0,X			;
	STAA	B,Y			;  Akzeptanzwert in Akzeptanzfilter eintragen
CHK_FILTER7:
	CLRA				;  mit A = 0 zurück
	BRA	CHK_FILTER9		;
					;
...
CHK_FILTER9:
	RTS				;
					;
;------------------------------------------------------------------------------
;ADD_FILTER trägt, sofern noch nicht vorhanden, das High Byte eines Identifiers
;in den nächsten freien Akzeptanzfilter ein.
;
;Eingangsparameter:	X		Zeiger auf Descriptor
;			Y               CAN Register-Basisadresse
...
;begin 09.01.2018
	...
	LDAB	#0			;
					;
	STAA	IDAR0,Y			;sämtliche Akzeptanzfilter auf 0FFh und
	STAB	IDMR0,Y			;sämtliche Akzeptanzmasken auf null setzen
	STAA	IDAR1,Y			;
	STAB	IDMR1,Y			;
	STAA	IDAR2,Y			;
	STAB	IDMR2,Y			;
	...
	STAB	IDMR7,Y			;
					;
	MOVB	#8,R3			;Anzahl der Descriptoren
					;
CAN_CONFIG1:
	LDAA	1,X			;
	ANDA	#00001111b		;wenn Anzahl Bytes = 0,
	BNE	CAN_CONFIG2		;dann
	JSR	ADD_FILTER		;  Akzeptanzfilter eintragen
CAN_CONFIG2:
	LEAX	2,X			;
	DEC	R3			;
	BNE	CAN_CONFIG1		;
