...
;
;28.12.2014	Korrektur in GET_DATA_HANDLE: Am Ende RTS ergänzt
;
...
;begin 20.01.2011
 if fBlockRead == _true
;-----------------------------------------------------------------------------
;A_CMD	empfängt Parameter für nachfolgende Blockdatenübertragung an den PC.
;-----------------------------------------------------------------------------
					;
A_CMD:
...
B_CMD1:
	...
	DBNE	D,B_CMD1		;
	STX	PB_OFFSET		;nächste Quelladresse merken
	BRA	B_CMD3			;
					;
;------------------------------------------------------------------------------
...
B_CMD2:
	...
	MOVB	R8,PB_PAGE+1		;nächste Quelladresse merken
	STX	PB_OFFSET		;
					;
;------------------------------------------------------------------------------
;Prüfcode bilden, dem Datensektor anhängen und mit übertragen
;
B_CMD3:
	LDX	#SCI_SECTOR_BUF		;
	LDD	#SCI_SECTOR_SIZE	;
	STD	R2			;
	JSR	CREATE_X25		;CRC-16/X25-Prüfcode berechnen
	MOVW	R0,SCI_SECTOR_CRC	;und speichern
	LDD	#SCI_SECTOR_SIZE	;
	ADDD	#2			; + 2 für den Prüfcode
	STD	SCI_SECTOR_CTR		;Sektor-Bytezähler auf Startwert
					;
	MOVW	#SCI_SECTOR_BUF,IO_PTR	;Zeiger auf Datenquelle
	LDD	#SCI_SECTOR_SIZE	;
	ADDD	#1			; + 2 für den Prüfcode
...
;begin 20.01.2011
 if fE_CMD_CALLBACK = _true
	JSR	COMMAND_EXECUTE		;Kommando ausführen...
 endif
;end
;
					;
...
;begin 20.01.2011
 if fCalibration == _true
;-----------------------------------------------------------------------------
;X_CMD	aktualisiert den Prüfcode der Kaliberierungstabellen.
;-----------------------------------------------------------------------------
					;
X_CMD:
	LDX	#E_CALIBRATION_TBL	;Prüfcode der Kalibrierungstabellen
